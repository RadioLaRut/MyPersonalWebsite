    ...
    return (
        <main className="w-full min-h-screen bg-black text-white pb-0 overflow-x-hidden mx-auto font-display selection:bg-white selection:text-black">
            ...
            <nav className="fixed top-0 left-0 w-full z-50 mix-blend-difference px-8 py-8 flex justify-between items-center pointer-events-none">
                <a href="/works" className="flex items-center gap-4 pointer-events-auto hover:opacity-70 transition-opacity interactive">
                    ...
                    <span className="font-mono text-xs tracking-widest text-white/80">INDEX</span>
                </a>
            </nav>

            {/* 1. Hero Section */}
            <header className="relative w-full h-[85vh] flex items-center justify-center overflow-hidden bg-black" ref={containerRef}>
                <motion.div
                    className="absolute inset-0 w-full h-full"
                    style={{ y: heroY }}
                >
                    <img src={data.heroImage} alt={data.title} className="w-full h-full object-cover opacity-70" />
                ...
                <div className="relative z-10 text-center flex flex-col items-center gap-4 fade-in-section opacity-0 translate-y-8 transition-all duration-1000">
                    ...
                    {data.navLink && (
                        <a
                            href={data.navLink} target="_blank" rel="noopener noreferrer"
            ...
            <section className="max-w-[1920px] mx-auto px-8 md:px-16 pb-32 relative z-20 bg-black">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 fade-in-section opacity-0 translate-y-8 transition-all duration-1000 ease-out pt-12 border-t border-white/10">
                    ...
                    <div className="space-y-4">
                        <h4 className="text-white text-base font-bold tracking-widest uppercase border-l-2 pl-3 border-white/80 leading-none">{data.col1.title}</h4>
                        ...
                        <div className="aspect-cinema w-full relative overflow-hidden mt-4 border border-white/10 group">
                            <img src={data.col1.img} alt={data.col1.title} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity duration-700" />
                        </div>
                    </div>

                    <div className="space-y-4">
                        <h4 className="text-white text-base font-bold tracking-widest uppercase border-l-2 pl-3 border-white/80 leading-none">{data.col2.title}</h4>
                        <p className="text-white/60 text-sm font-normal leading-[1.8] font-sans">
                            {data.col2.text}
                        </p>
                        <div className="aspect-cinema w-full relative overflow-hidden mt-4 border border-white/10 group">
                            <img src={data.col2.img} alt={data.col2.title} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity duration-700" />
                        </div>
                    </div>

                    <div className="space-y-4">
                        <h4 className="text-white text-base font-bold tracking-widest uppercase border-l-2 pl-3 border-white/80 leading-none">{data.col3.title}</h4>
                        <p className="text-white/60 text-sm font-normal leading-[1.8] font-sans">
                            {data.col3.text}
                        </p>
                        <div className="aspect-cinema w-full relative overflow-hidden mt-4 border border-white/10 group">
                            <img src={data.col3.img} alt={data.col3.title} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity duration-700" />
                        </div>
                    </div>
            ...
            {data.gallery && data.gallery.length > 0 && (
                <section className="w-full flex flex-col pt-0 pb-0 gap-y-1 bg-black relative z-10">
                    {data.gallery.map((imgSrc: string, index: number) => (
                        <div key={index} className="w-full h-screen relative bg-black fade-in-section opacity-0 translate-y-8 transition-all duration-1000 ease-out">
                            <img src={imgSrc} alt={`${data.title} Gallery ${index}`} className="absolute inset-0 w-full h-full object-cover opacity-100 pointer-events-none" />
                        </div>
                    ))}
                </section>
            )}
            ...
            <footer className="mt-0 border-t border-white/20 relative z-20">
                <a href={`/works/${data.nextId}`} className="group block relative h-[40vh] md:h-[60vh] overflow-hidden w-full interactive bg-black">
                    <div className="absolute inset-0 bg-black z-10 transition-colors duration-500"></div>
                    <img src={data.nextBg} alt="Next Project" className="absolute inset-0 w-full h-full object-cover scale-100 group-hover:scale-105 transition-transform duration-[2s] ease-out opacity-40 group-hover:opacity-60" />
